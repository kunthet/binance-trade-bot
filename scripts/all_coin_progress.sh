...
while read p; do
   ...
   then
	...
	echo $first_date
     ...
     echo $last_value
     echo -n "Grow: "
     grow=$(awk 'BEGIN { if($first_value>0) {print ($last_value/$first_value*100)-100} else { print 0 }}')
     echo -n $grow
     echo "%"	
	echo -n "Starting value: "
	...
	echo -n $first_value2
     ...
     growb=$(awk 'BEGIN { if($first_value2>0){print ($last_valueb/$first_value2*100)-100} else {print 0 } }')
     grows=$(awk 'BEGIN { if($first_value2>0){print ($last_values/$first_value2*100)-100} else {print 0 } }')
     echo -n $growb
     echo -n "% / "
     echo -n $grows
     echo "%"
